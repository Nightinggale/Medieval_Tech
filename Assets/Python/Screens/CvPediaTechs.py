...
class CvPediaTechs:
	...
	def interfaceScreen(self, iCivic, x, y, h, w):
		...
		# Top
		screen.setText(self.top.getNextWidgetName(), "Background", self.top.MENU_TEXT, CvUtil.FONT_LEFT_JUSTIFY, self.top.X_MENU, self.top.Y_MENU, 0, FontTypes.TITLE_FONT, WidgetTypes.WIDGET_PEDIA_MAIN, CivilopediaPageTypes.CIVILOPEDIA_PAGE_TECHNOLOGY, -1)
		if self.top.iLastScreen	!= CvScreenEnums.PEDIA_TECHS or bNotActive:
			self.placeLinks(true)
			self.top.iLastScreen = CvScreenEnums.PEDIA_TECHS
		else:
			self.placeLinks(true)
	...
	def placeTechStats(self):
		...
		screen.appendListBoxString(panelName, u"<font=4>" + localText.getText("TXT_KEY_PEDIA_CATEGORY_TECHS", ()).upper() + u"</font>", WidgetTypes.WIDGET_GENERAL, 0, 0, CvUtil.FONT_LEFT_JUSTIFY)
	...
	def placeLinks(self, bRedraw):
		...
		for iI in range(gc.getNumCivicInfos()):
			if (not gc.getCivicInfo(listSorted[iI][1]).isGraphicalOnly()):
				...
				#TKe
				if bRedraw:
					screen.appendListBoxString(self.top.LIST_ID, listSorted[iI][0], WidgetTypes.WIDGET_PEDIA_JUMP_TO_TECH, listSorted[iI][1], 0, CvUtil.FONT_LEFT_JUSTIFY )
