...
class CvPediaTradeTechs:
	"Civilopedia Screen for Trade Techs"
	...
	def interfaceScreen(self, iCivic, x, y, h, w):
		...
		# Top
		screen.setText(self.top.getNextWidgetName(), "Background", self.top.MENU_TEXT, CvUtil.FONT_LEFT_JUSTIFY, self.top.X_MENU, self.top.Y_MENU, 0, FontTypes.TITLE_FONT, WidgetTypes.WIDGET_PEDIA_MAIN, CivilopediaPageTypes.CIVILOPEDIA_PAGE_TRADE_TECHS, -1)
		if self.top.iLastScreen	!= CvScreenEnums.PEDIA_TRADE_TECHS or bNotActive:
			self.placeLinks(true)
			self.top.iLastScreen = CvScreenEnums.PEDIA_TRADE_TECHS
		else:
			self.placeLinks(true)
	...
	def placeStats(self):
		...
		screen.appendListBoxString(panelName, u"<font=4>" + localText.getText("TXT_KEY_PEDIA_CATEGORY_TRADE_PERKS", ()).upper() + u"</font>", WidgetTypes.WIDGET_GENERAL, 0, 0, CvUtil.FONT_LEFT_JUSTIFY)
	...
	def placeLinks(self, bRedraw):
		...
		for j in range(gc.getNumCivicInfos()):
			if gc.getCivicInfo(j).getInventionCategory() == 2:
		...
		for iI in range(gc.getNumCivicInfos()):
			if (not gc.getCivicInfo(listSorted[iI][1]).isGraphicalOnly()):
				#TKs
				if gc.getCivicInfo(listSorted[iI][1]).getInventionCategory() != 2:
					continue
				#TKe
				if bRedraw:
					screen.appendListBoxString(self.top.LIST_ID, listSorted[iI][0], WidgetTypes.WIDGET_PEDIA_JUMP_TO_TRADE_TECH, listSorted[iI][1], 0, CvUtil.FONT_LEFT_JUSTIFY )
